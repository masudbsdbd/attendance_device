<el-dialog>
    <dialog id="dialog_new_{{ $user['uid'] }}" aria-labelledby="dialog-title" 
            ...
            <el-dialog-panel class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all 
                ...
                <div class="p-6 sm:p-8">
                    <form action="{{ route("edituser", $current_device_id) }}" method="POST" class="space-y-6">
                        ...
                        <!-- Card No. -->
                        <div>
                            <label for="cardno" class="block text-sm font-medium text-gray-700 mb-1.5">
                                Full Name
                            </label>
                            ...
                            <div class="relative">
                                <select id="role" name="role" 
                                    ...
                                    @foreach($roles as $role)
                                        <option {{ $role->role_id == $user['role'] ? "selected" : "" }} value="{{ $role->role_id }}">{{ $role->role_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="pt-4">
                            <button type="submit" 
                                    class="cursor-pointer w-full flex justify-center items-center gap-2 rounded-xl bg-gradient-to-r from-green-600 to-green-600 
